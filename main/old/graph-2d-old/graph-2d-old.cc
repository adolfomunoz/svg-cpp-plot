...
int main(int argc, char** argv) {
	SVG svg;
	Graph2DOld& graph = svg.add(Graph2DOld({100,100},BoundingBox(-2*M_PI,-1,2*M_PI,1)));
	graph.id("graph-2d");
	graph.all_plots().stroke_width(1).hover().stroke_width(2);
	graph.plot_function([] (float x) { return sin(x); }).stroke(yellow);
	graph.plot_function([] (float x) { return 1.3*cos(x); }).stroke(red);
